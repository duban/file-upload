...
const { parseDateTime } = require('./util');
...
function Parse(opts) {
  ...
  this._pullStream.once("finish", function () {
    self.emit('cd.entries', self.entries);
...
Parse.prototype._readCentralDirectoryFileHeader = function () {
  ...
  this._pullStream.pull(42, function (err, data) {
    ...
    var vars = binary.parse(data)
      ...
      .word16lu('flags')
      .word16lu('compressionMethod')
      .word16bu('lastModifiedTime')
      .word16bu('lastModifiedDate')
      .word32lu('crc32')
      .word32lu('compressedSize')
      .word32lu('uncompressedSize')
      .word16lu('fileNameLength')
    ...
    return self._pullStream.pull(vars.fileNameLength, function (err, fileName) {
      ...
      self._pullStream.pull(vars.extraFieldLength, function (err, extraField) {
        ...
        self._pullStream.pull(vars.fileCommentLength, function (err, fileComment) {
          ...
          fileName = fileName.toString('utf8');
          const lastModifiedData = data.slice(8, 12);
          const lastModified = parseDateTime(lastModifiedData);
          const info = {
            path: fileName,
            compressedSize: vars.compressedSize,
            uncompressedSize: vars.uncompressedSize,
            lastModified,
            type: (vars.compressedSize === 0 && /[\/\\]$/.test(fileName)) ? 'Directory' : 'File',
          };
          self.entries.push(info);
          self.emit('cd.entry', info);
          return self._readRecord();
        });
      });
    });
